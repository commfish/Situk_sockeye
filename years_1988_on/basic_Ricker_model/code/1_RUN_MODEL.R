...
# create output folder for model results
out.path <- paste0("years_1988_on/basic_Ricker_model/output/processed/")
if(!exists(out.path)){dir.create(out.path)}
...
source("years_1988_on/basic_Ricker_model/code/model_data.R")

# generates initial values
source("years_1988_on/basic_Ricker_model/code/model_inits.R")
...
if(package.use == "rjags" & sensitivity.analysis == 0){
  ...
  jmod <- rjags::jags.model(
    file='years_1988_on/basic_Ricker_model/code/Situk_sockeye.txt', 
    data = dat, n.chains = 3, 
    inits = inits, 
    n.adapt = n.adapt.use) 
