...
pdf("years_1988_on/basic_Ricker_model/output/geweke.pdf",height=10, width=8,onefile=T)
geweke.plot(post)
dev.off()
...
out.file <- paste0("years_1988_on/basic_Ricker_model/output/density.png")
...
post_df <- as.data.frame(as.mcmc(do.call(rbind, post))) %>%
  ...
  pivot_longer(
    ...
    names_to = "parameter",
    values_to = "value")
out.file <- paste0("years_1988_on/basic_Ricker_model/output/posterior_densities.png")
...
out.file <- paste0("years_1988_on/basic_Ricker_model/output/trace_plots.png")
...
out.file <- paste0("years_1988_on/basic_Ricker_model/output/dens_plots.png")
...
write.csv(out, file = "years_1988_on/basic_Ricker_model/output/processed/recruit_data.csv", row.names = FALSE)

# create param table
read.csv("years_1988_on/basic_Ricker_model/output/quantiles_lambert.csv") %>%
  dplyr::select(variable, X2.5, X50, X97.5, post_cv) %>%
  write.csv(., file = "years_1988_on/basic_Ricker_model/output/processed/params.csv", row.names = FALSE)

# ACF plots
pdf("years_1988_on/basic_Ricker_model/output/ACF_parameters.pdf", width = 7, height = 5)
for(p in parameters){
